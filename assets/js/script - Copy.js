const items = document.getElementById('food-item');
let food_arr = [];
...
function showCategoryItems(item) {
    ...
    $('.fd-list .remove-me').on('click', function(e) {
    ...
    if (val.length > 0) {
        ...
        $('#results').html(`
						<table id='food-raw' class='table table-bordered table-striped'>
							<caption class='table-caption'><button id='prep' class='btn btn-block btn-sm btn-success'>PREP</button> </caption>
							<thead>
								<tr>
    ...
    $('#prep').on('click', function() {
        ...
        $('#cook-now').on('click', function() {
            ...
            $('#result-items').html(`
                                    ...
                                    <!--div>Items: ${JSON.stringify(food_items)}</div-->
